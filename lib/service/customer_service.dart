...
class CustomerService {
  ...
  Future<String?> linkGoogle({required String googleId}) async {
    final baseUrl = await _getBaseUrl();
    final customerId = await getCustomerId();

    final googleId = await Session().getgoogleId();
    ...
    final res = await http.post(
      ...
      headers: {
        ...
        if (googleId != null && googleId.isNotEmpty) 'Authorization': 'Bearer $googleId',
      },
      body: jsonEncode({'idgoogleId': googleId}),
    );
